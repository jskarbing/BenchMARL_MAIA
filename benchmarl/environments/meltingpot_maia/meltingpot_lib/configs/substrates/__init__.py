...
# import meltingpot.configs.substrates

# Slightly modified version of meltingpot/configs/substrates/__init__.py with:
...
def get_config(substrate: str) -> config_dict.ConfigDict:
  ...
  if substrate in ['harvest__maia']:
    path = f'{__name__}.{substrate}'
  else:
    path = f'meltingpot.configs.substrates.{substrate}'
...
SUBSTRATES: Set[str] = frozenset({
    # MAIA custom substrates. keep-sorted start
    'harvest__maia',
    # MAIA custom substrates. keep-sorted end 

    # original substrates. keep-sorted start
    ...
    'territory__open',
    'territory__rooms',
    # original substrates. keep-sorted end
})
