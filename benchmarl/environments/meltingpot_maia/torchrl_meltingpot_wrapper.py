...
# import torchrl.envs.libs.meltingpot

# Slightly modified version of torchrl/envs/libs/meltingpot.py with
#   Copyright (c) Meta Platforms, Inc. and affiliates.
#   License found in the LICENSE file in the root directory of the torchrl source tree.
...
def _get_envs():
    ...
    from BenchMARL_MAIA.benchmarl.environments.meltingpot_maia.meltingpot_lib.configs import substrates as substrate_configs
...
def _filter_collective_reward_from_dict(obs_dict: Dict) -> Dict:
    return {
        key: value
        for key, value in obs_dict.items()
        if "COLLECTIVE_REWARD" not in key
    }

def _filter_from_dict(obs_dict: Dict, world: bool) -> Dict:  # noqa
    return {
        key: value
        for key, value in obs_dict.items()
        if ((_WORLD_PREFIX not in key and "COLLECTIVE_REWARD" not in key) if not world else (_WORLD_PREFIX in key))
    }

def _remove_collective_reward_observations_from_obs_spec(
    observation_spec: Sequence[Mapping[str, "dm_env.specs.Array"]],  # noqa
) -> Sequence[Mapping[str, "dm_env.specs.Array"]]:  # noqa
    return [
        _filter_collective_reward_from_dict(agent_obs)
...
def _remove_world_observations_from_obs_spec(
    ...
    return [
        _filter_from_dict(agent_obs, world=False)
        for agent_obs in observation_spec
    ]
...
def _global_state_spec_from_obs_spec(
    ...
    world_entries = _filter_from_dict(observation_spec[0], world=True)
...
class MeltingpotWrapper(_EnvWrapper):
    ...
    def _make_specs(
        ...
        mp_obs_spec_no_collective_reward = _remove_collective_reward_observations_from_obs_spec(
            mp_obs_spec_no_world
        )  # List of dict of arrays
        ...
        torchrl_agent_obs_specs = [
            ...
            for agent_obs_spec in mp_obs_spec_no_collective_reward
        ]
    ...
    def _reset(
        ...
        # Global state
        td.update(
            _remove_world_prefix(_filter_from_dict(obs[0], world=True))
        )
...
class MeltingpotEnv(MeltingpotWrapper):
    ...
    def _build_env(
        ...
        from benchmarl.environments.meltingpot_maia.meltingpot_lib import substrate as mp_substrate
